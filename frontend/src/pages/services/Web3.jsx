  ...
  return (
    <div className="w-full pb-32">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        ...
        <div className="w-full h-64 md:h-96 glass-panel rounded-[3rem] mb-16 relative flex items-center justify-center overflow-hidden border border-boraq-gray-silver/10 dark:border-boraq-teal-deep/10">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(130,169,180,0.1)_0%,transparent_70%)]" />
          ...
          <div className="flex gap-4 md:gap-8 items-center z-10 relative">
            <motion.div
              ...
              className="w-20 h-20 md:w-32 md:h-32 bg-boraq-black/40 backdrop-blur-md rounded-xl md:rounded-2xl border border-boraq-teal-steel/50 rotate-45 transform-gpu shadow-[0_0_30px_rgba(130,169,180,0.3)] flex items-center justify-center"
            >
              <Blocks className="w-8 h-8 md:w-12 md:h-12 -rotate-45 text-boraq-teal-steel opacity-80" />
            </motion.div>
            <motion.div
              animate={{ y: [0, 20, 0] }} transition={{ repeat: Infinity, duration: 5, ease: "easeInOut", delay: 1 }}
              className="w-24 h-24 md:w-40 md:h-40 bg-boraq-black/40 backdrop-blur-lg rounded-xl md:rounded-2xl border border-boraq-teal-steel bg-boraq-teal-steel/10 rotate-45 transform-gpu shadow-[0_0_50px_rgba(130,169,180,0.5)] z-10 flex items-center justify-center"
            >
              <Globe className="w-10 h-10 md:w-16 md:h-16 -rotate-45 text-boraq-white opacity-90" />
            </motion.div>
            <motion.div
              animate={{ y: [0, -15, 0] }} transition={{ repeat: Infinity, duration: 4.5, ease: "easeInOut", delay: 0.5 }}
              className="w-20 h-20 md:w-32 md:h-32 bg-boraq-black/40 backdrop-blur-md rounded-xl md:rounded-2xl border border-boraq-teal-steel/50 rotate-45 transform-gpu shadow-[0_0_30px_rgba(130,169,180,0.3)] flex items-center justify-center"
            >
              <Lock className="w-8 h-8 md:w-12 md:h-12 -rotate-45 text-boraq-teal-steel opacity-80" />
            </motion.div>
          </div>
          {/* Connecting Lines */}
          <div className="absolute top-1/2 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-boraq-teal-steel/30 to-transparent" />
        </div>
